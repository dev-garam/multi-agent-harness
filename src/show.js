...
export function summarizeRunManifest(manifest) {
  ...
  return {
    ...
    usage: manifest.usageSummary || agentUsageSummary(manifest.steps || []),
...
export function formatRunSummary(summary) {
  const lines = [
    ...
    `Pipeline selection: ${summary.pipelineSelection ? `${summary.pipelineSelection.mode} selected=${summary.pipelineSelection.selected} risk=${summary.pipelineSelection.riskScore} complexity=${summary.pipelineSelection.complexityScore}` : '(none)'}`,
    ...
    '',
    'Usage',
    `Provider calls: ${summary.usage.providerCalls ?? '(unknown)'}${summary.usage.maxProviderCalls ? ` / ${summary.usage.maxProviderCalls}` : ''}`,
    `Remaining provider calls: ${formatNullable(summary.usage.remainingProviderCalls)}`,
