...
`reporter`가 쓰는 내용의 재료는 이미 전부 manifest에 있습니다 — 변경 파일(inspection), 검증 결과, 감독 판단과 지시, 사용량. LLM은 그걸 산문으로 옮겨 적을 뿐인데, 실측에서 그 작업이 run 비용의 **30.9%**를 썼습니다.

```json
{
  "reporter": { "mode": "deterministic" }
}
```

켜면 `reporter` 스텝이 provider를 호출하지 않고 manifest에서 보고서를 생성합니다. 산출물 계약은 동일합니다(사람이 읽는 markdown + 마지막 fenced JSON). 보고 내용이 manifest와 어긋날 수 없다는 부수 효과도 있습니다.

이때 `hermes`도 압축된 프롬프트(`prompts/hermes-concise.md`)를 씁니다. 결정론 reporter는 산문을 읽지 않으므로 감독이 긴 보고서를 쓸 이유가 없기 때문입니다. **평가 지시와 결정 규칙은 그대로 두고 출력 형식만 압축**합니다 — 감독의 추론 깊이는 줄이지 않습니다. 사용자가 알아야 할 내용은 `reason`과 `instructions`에 담기고, 그 두 필드는 최종 보고서에 그대로 인용됩니다.

실측(README 한 줄 수정, 실제 claude):

| 역할 | 기본 | 결정론 reporter |
| --- | ---: | ---: |
| coder | $0.1735 | $0.1739 |
| hermes | $0.3054 (output 4,495) | $0.1926 (output 1,245) |
| reporter | $0.2146 | **$0.0000** |
| **합계** | **$0.6935** | **$0.3665** |

**47.2% 절감**이고 coder 비용은 그대로입니다. 감독 품질도 유지됩니다 — 축소 프롬프트에서도 hermes는 `git diff`, `grep`, `git status`로 증거를 직접 대조했습니다.
...
약한 모델(Haiku)로도 재봤습니다.

| 조건 | 통과 | 비용 |
| --- | --- | ---: |
| Opus solo | 2/2 | $0.532 |
| Opus harness | 2/2 | $1.642 |
| Haiku solo | 2/2 | **$0.118** |
| Haiku harness | 2/2 | $0.239 |

**네 조건 모두 통과했고, `solo Haiku`가 `harness Opus`보다 14배 쌉니다.** 이 난이도에서는 하네스가 강한 모델에게도 약한 모델에게도 순수 오버헤드입니다.

이 벤치마크는 "고쳤는가" 하나만 잽니다. 감사 추적, 정책 게이트, 격리 실행, 재현 가능성 같은 하네스의 다른 가치는 측정 대상이 아닙니다. 위 결과는 **하네스를 성능 도구가 아니라 통제 도구로 봐야 한다**는 뜻으로 읽는 것이 정확합니다.
