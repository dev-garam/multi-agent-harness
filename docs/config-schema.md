...
- `supervisor.forceRevalidateOnFailure` (default `true`) turns a `continue` decision into one forced `run_validation` when validation failures are still open. Without it that combination is a dead end: the harness fails the run anyway, so the decision is neither honoured nor recovered from. The override happens at most once per run — unlimited retries would loop — and only while `maxSupervisorTurns` budget remains. The original decision and the override are both recorded in `supervisorDecisions[]`.
