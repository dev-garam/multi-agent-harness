...
**증폭 가설은 아직 테스트되지 않았다.** 하네스가 값어치를 하려면 셋 중 하나여야 한다.

1. 약한 모델을 강한 모델 수준으로 끌어올린다 (증폭) — 아직 못 쟀다. `gemini` CLI가 설치돼 있으나 실행이 깨져 있고(node 경로), `bench.mjs`의 solo 모드도 현재 claude만 지원한다.
2. 강한 모델도 실패하는 난이도에서 재시도·검증으로 성공률을 올린다 — 그런 fixture가 아직 없다.
3. 결과가 아니라 안전이 목적인 상황(정책 게이트, 감사)에서 값을 한다 — 이 벤치마크의 측정 대상이 아니다.

다음 라운드에 필요한 것:

| 항목 | 이유 |
|------|------|
| `gemini` CLI 수리 + solo 모드 다중 provider 지원 | 증폭 가설의 전제 |
| Claude가 실패하는 fixture | 천장 효과 회피. 여러 파일에 걸친 결함, 재현에 조건이 필요한 버그 등 |
| 하네스 최적화 설정으로 재측정 | 이번 fixture는 기본 설정(agent reporter)이었다. `reporter.mode: deterministic`이면 3.1배가 줄어든다 |
