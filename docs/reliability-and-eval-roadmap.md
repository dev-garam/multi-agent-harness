...
### 이어받기(--continue) 추가 (2026-08-21)

실사용 피드백에서 나왔다. 하네스를 돌려놓고 응답을 기다리는 동안(실측 243초, 그중 84%가 coder) 추가 지시가 생기는데 반영할 방법이 없었다.

실행 중인 스텝에 끼어드는 것은 구조적으로 불가능하다. CLI가 이미 렌더된 프롬프트를 받아 돌고 있고 `stdio`의 stdin은 `ignore`다. 대화형으로 바꾸려면 실행 모델 자체가 달라진다.

대신 **끝난 run 위에 쌓는 것**은 재료가 이미 있었다: `changes.patch`(변경)와 manifest(맥락). `harness run --continue [runId]`가 둘을 이어받는다. 요약만 넣고 스텝 출력 전문은 넣지 않는다 — 이미 끝난 대화이고 다시 넣으면 컨텍스트만 부풀린다.

**두 시나리오를 구분해야 한다.**

| 상황 | 처리 |
|------|------|
| "A 고쳐줘" → "B도 같이" | `--continue`. 이전 변경 위에 쌓는다 |
| "A 고쳐줘" → "아니 A 말고 B" | **그냥 새 run.** 이미 되던 것이다 |

두 번째가 롤백 없이 되는 것은 오늘 기본값을 `worktree`로 바꾼 덕이다. 이전 변경이 원본에 반영된 적이 없으므로 새 run을 돌리면 자동으로 없던 일이 된다. `direct`가 기본이던 시절이라면 A가 원본에 이미 써져 있어 되돌려야 했고, 그 과정에서 작업 중이던 다른 변경까지 날아갔을 것이다.

구현 중 CLI 파싱 결함을 하나 잡았다. `--continue` 다음 인자를 무조건 소비해 `--continue "작업 요청"` 에서 요청문을 runId로 먹었다. runId 형식일 때만 소비하도록 고쳤다.
